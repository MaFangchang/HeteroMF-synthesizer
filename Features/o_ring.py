...
class ORing(MachiningFeature):
    ...
    def _add_sketch(self, bound, hetero):
        ...
        if hetero:
            ...
            edge1 = BRepBuilderAPI_MakeEdge(circ, 0., scale1 * math.pi).Edge()
            edge2 = BRepBuilderAPI_MakeEdge(circ, scale1 * math.pi, 2 * math.pi).Edge()
            outer_wire = BRepBuilderAPI_MakeWire(edge1, edge2).Wire()

            normal.Reverse()
            circ = gp_Circ(gp_Ax2(gp_Pnt(center[0], center[1], center[2]), normal), inner_r)
            edge1 = BRepBuilderAPI_MakeEdge(circ, 0., scale2 * math.pi).Edge()
            edge2 = BRepBuilderAPI_MakeEdge(circ, scale2 * math.pi, 2 * math.pi).Edge()
            inner_wire = BRepBuilderAPI_MakeWire(edge1, edge2).Wire()
