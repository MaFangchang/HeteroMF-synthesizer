...
def rotate_vector(vector, angle, axis):
    ...
    rotation_matrix_z = np.array([[np.cos(angle), -np.sin(angle), 0.],
                                  ...
                                  [0., 0., 1.]])
    if axis.IsParallel(gp_Dir(1., 0., 0.), 1e-6):
        rotate_vec = rotation_matrix_x.dot(vector)
    elif axis.IsParallel(gp_Dir(0., 1., 0.), 1e-6):
        rotate_vec = rotation_matrix_y.dot(vector)
    elif axis.IsParallel(gp_Dir(0., 0., 1.), 1e-6):
        rotate_vec = rotation_matrix_z.dot(vector)
    else:
        rotate_vec = vector
